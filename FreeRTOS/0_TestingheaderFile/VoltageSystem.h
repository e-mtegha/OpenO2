...
extern uint8_t SIGNAL_PIN;        // Frequency/voltage input pin (interrupt capable)

// ================= Shared Variables =================
extern float batteryVoltage;  // From analog read
extern float voltage;         // From frequency input
...
inline void taskBatteryVoltage(void *pv) {
    ...
    for (;;) {
        ...
        batteryVoltage = vOut * 3.0; // Assuming 1:3 voltage divider
...
inline void taskVoltageCapture(void *pv) {
    ...
    for (;;) {
        if (dataReady) {
            ...
            voltage = computeEMA(periodToVoltage(medianPeriod));
        }
